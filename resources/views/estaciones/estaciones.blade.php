            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="demoModalLabel">Creación de nueva Estacion</h5>
...
<!-- fin de modal ingreso nuevo-->
    <div class="table">
        <table id="customers">
            ...
            <tbody>
                @foreach($estaciones as $m)
                <tr>
                	<td>{{$m->nombre}}</td>
                    <td>{{$m->cant_usuarios}}</td>
                    <td>{{$m->nombre}}</td>
                    <td width= "10%">
                    	<a  href="{{route('estaciones.edit',$m->id)}}">Editar</a>
                    </td>
                    <td width= "10%">
                      <form method="post" action="{{ url('/estaciones/'.$m->id) }}">
                        {{csrf_field() }}
                        {{ method_field('DELETE') }}
